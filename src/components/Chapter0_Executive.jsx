...
const DashboardCard = ({ title, value, sub, trendValue, isPositiveGood = true, colorClass, barColor }) => {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] border border-slate-100 relative overflow-hidden group">
...
const Chapter0_Executive = ({ pnlData, onPnlChange, historyData, onSaveArchive }) => {
  ...
  const calculatedData = useMemo(() => {
    ...
    const totalRow = {
      id: 'total', name: '합계', ...sums, op: totalOp, prevOp: totalPrevOp,
        ...
        <div className="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div>
            ...
            <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
              <Calendar className="text-indigo-600"/> {selectedMonth.month} 월별 상세 실적
            </h2>
          </div>
          <div className="flex gap-6 text-right">
             <div>
                <p className="text-xs text-slate-500 uppercase font-bold">Total Rev</p>
                <p className="text-xl font-bold text-slate-800">₩ {historyTotal.rev.toFixed(1)}B</p>
             </div>
             <div>
                <p className="text-xs text-slate-500 uppercase font-bold">Total OP</p>
                <p className={`text-xl font-bold ${historyTotal.op >= 0 ? 'text-slate-800' : 'text-red-600'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
             <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2">
                <BarChart3 size={18} className="text-indigo-500"/> 사업부별 손익 구조 (P&L Breakdown)
             </h3>
             <table className="w-full text-sm text-left">
               <thead className="bg-slate-50 text-slate-500 border-y border-slate-200">
                 <tr>
                   <th className="py-3 px-4 font-semibold">사업부</th>
                   <th className="py-3 px-2 text-right font-semibold">매출 (B)</th>
                   <th className="py-3 px-2 text-right font-semibold">GM (B)</th>
                   <th className="py-3 px-2 text-right font-semibold">고정비 (B)</th>
                   <th className="py-3 px-2 text-right font-semibold">비율 (%)</th>
           ...
           <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
             <h3 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><PieIcon size={18}/> 비용 구성 (Cost Mix)</h3>
             <div className="h-64">
               {selectedMonth.cost_details && selectedMonth.cost_details.length > 0 ? (
                 ...
                 <div className="flex items-center justify-center h-full text-slate-400 text-sm">No Cost Details</div>
               )}
             </div>
           </div>
        </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 2. 손익 구조 (Auto-Sort & Editable Table) */}
              ...
              <thead>
                <tr className="bg-slate-50 text-slate-500 border-y border-slate-200 cursor-pointer select-none">
                  ...
                  <th className="py-3 px-2 text-right font-semibold w-24 hover:bg-slate-100" onClick={() => handleSort('rev')}>
                    매출 (B) <ArrowUpDown size={12} className="inline ml-1"/>
                  </th>
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-slate-50/50 transition align-top">
                      ...
                      <td className="py-2 px-2">
                        <div className="flex items-center justify-end gap-1 bg-white border border-slate-200 rounded px-2 py-0.5 w-full focus-within:ring-2 focus-within:ring-indigo-500">
                          <input type="number" step="0.01" className="w-full text-right outline-none text-slate-900 font-bold bg-transparent text-sm"
                      ...
                      {/* [EDITABLE] 고정비 */}
                      <td className="py-2 px-2">
                        <div className="flex items-center justify-end gap-1 bg-white border border-slate-200 rounded px-2 py-0.5 w-full focus-within:ring-2 focus-within:ring-amber-500">
                          <input type="number" step="0.01" className="w-full text-right outline-none text-slate-600 font-medium bg-transparent text-sm"
        ...
        {/* 3. Monthly Archive (Drill-down Trigger) & Save Button */}
        <div className="lg:col-span-1 bg-white rounded-2xl p-6 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.07)] border border-slate-100 flex flex-col">
          <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
            <Calendar className="text-slate-500" size={20}/> 월별 실적 아카이브
          </h3>
          ...
          <div className="mb-4 bg-indigo-50 p-3 rounded-xl border border-indigo-100">
             <div className="flex gap-2">
                <input 
                  type="month" 
                  className="flex-1 text-sm border border-indigo-200 rounded-lg px-2 outline-none focus:border-indigo-500"
                  value={saveMonthName}
             ...
             <p className="text-[10px] text-indigo-400 mt-1.5 pl-1">
               * 현재 테이블 값을 {saveMonthName} 실적으로 저장합니다.
             </p>
          </div>
